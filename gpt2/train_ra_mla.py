...
parser.add_argument("--latent-dim", type=int, default=64, help="Latent dimension for K/V compression")
parser.add_argument("--ra-window", type=int, default=64, help="Reciprocal attention band width")
parser.add_argument("--ra-alpha", type=float, default=0.5, help="Reciprocal attention weight (0.0 = pure MLA)")
parser.add_argument("--per-head-q-latent", action="store_true", default=True, help="Per-head Q-to-latent projection")
parser.add_argument("--per-head-v-up", action="store_true", default=True, help="Per-head V up-projection")
parser.add_argument("--use-flash", action="store_true", default=True, help="Use FlashAttention if available")
...
def get_batch(split: str, batch_size: int, block_size: int, device: str, data_dir: str = "data"):
    ...
    if not os.path.exists(data_path):
        print(f"Warning: Data path {data_path} not found, using random data for testing")
...
class RAMLAMetrics:
    ...
    def log(self, iter: int, model: GPT, forward_time_ms: float):
        ...
        self.memory_allocated.append(torch.cuda.memory_allocated() / 1024**2 if torch.cuda.is_available() else 0)
...
def get_lr(iter: int, warmup_iters: int, max_iters: int, lr: float, min_lr: float) -> float:
...
@torch.no_grad()
def estimate_loss(model, eval_samples: int, batch_size: int, block_size: int, device: str):
...
def main():
    ...
    print(f"  latent_dim={args.latent_dim}, ra_window={args.ra_window}, ra_alpha={args.ra_alpha}")
    print(f"  per_head_q_latent={args.per_head_q_latent}, per_head_v_up={args.per_head_v_up}")
    ...
    # Create optimizer
    optimizer, scheduler, gradient_clip_norm, spam_state, adamprune_state = create_optimizer(
        model,
        args.optimizer,
        args.learning_rate,
        num_epochs=None,
    ...
    print(f"Optimizer: {args.optimizer}, LR: {args.learning_rate}, weight_decay: {args.weight_decay}")
    ...
    while iter_num < args.max_iters:
        ...
        lr = get_lr(iter_num, args.warmup_steps, args.max_iters, args.learning_rate, args.min_lr)
        ...
        for micro_step in range(args.gradient_accumulation):
            x, y = get_batch('train', args.batch_size, args.block_size, device, args.data_dir)
    ...
    # Final evaluation
    print("\nFinal evaluation:")
    final_losses = estimate_loss(model, args.eval_samples, args.batch_size, args.block_size, device)
    ...
    if metrics.attention_entropy:
        print(f"\nAttention Metrics:")
        print(f"  Entropy: {np.mean(metrics.attention_entropy):.3f} ± {np.std(metrics.attention_entropy):.3f}")
    if metrics.reciprocity_score:
