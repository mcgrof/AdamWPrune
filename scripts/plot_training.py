...
def plot_single_training(metrics, output_prefix="training"):
    ...
    # Handle different JSON structures (LeNet-5 vs ResNet-18)
    if "epochs" in metrics:
        ...
        avg_losses = [
            e.get("avg_loss", 0) for e in epochs_data if e.get("avg_loss", 0) > 0
        ]
        epoch_times = [e.get("epoch_time", 0) for e in epochs_data]
    elif "test_accuracy" in metrics:
        # ResNet-18 style (lists)
        epochs = list(range(1, len(metrics["test_accuracy"]) + 1))
        accuracies = metrics["test_accuracy"]
        avg_losses = metrics.get("train_loss", [])
        epoch_times = metrics.get("epoch_time", [])
    else:
        print("Error: Unrecognized JSON format")
        return

    # Extract sparsity data if available
    sparsities = []
    if "epochs" in metrics:
        # LeNet-5 style
        sparsities = [e.get("sparsity", 0) * 100 for e in epochs_data]
    elif "sparsity" in metrics:
        # ResNet-18 style
        sparsities = [s * 100 for s in metrics["sparsity"]]

    # Create figure with subplots - now 2x3 for additional loss detail plot
    fig, axes = plt.subplots(2, 3, figsize=(20, 10))
    # Determine model name from config if available
    model_name = "Neural Network"
    if "config" in metrics:
        if "model" in metrics["config"]:
            model_name = metrics["config"]["model"].upper()
