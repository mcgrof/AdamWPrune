...
## Novel Attention Mechanisms: Reciprocal Attention (RA) and Cooperative Grouped Attention (CoGA)

### Reciprocal Attention (RA): Data Wants to be Found

Traditional self-attention forces every token to attend to all others, resulting in O(n²) complexity. Reciprocal Attention (RA) introduces a fundamentally different paradigm: **allow attention to form reciprocally based on mutual attraction**.
...
- **Standard attention**: O(n²) - every token attends to all previous tokens
- **RA with local band**: O(nW) where W << n - reciprocal scoring only within window W
...
3. **Query-to-Latent Alignment**:
   ```
   Q_h_latent = project(Q_h, L)  ∈ ℝ^(n×L)
   ```
...
This potentially brings training complexity from O(n²) toward O(nL) by:
...
### RA/CoGA Implementation Status

We've implemented a reference RA+MLA architecture in `gpt2/ra_mla_gpt2.py` combining:
- Low-rank latent K/V compression (inspired by DeepSeek MLA)
- Reciprocal symmetric scoring within local bands
- Per-head value expansion for expressiveness
- AdamWPrune integration for pruning low-score heads

CoGA is currently in the design phase - we're exploring clustering algorithms and aggregation strategies.
